...
import { OpenFile, OpenImportFile, ReadFile, ReadFileBytes, SaveFile, SaveFileAs, GetStartupFile, StartWatching, StopWatching, CheckForUpdates, ConvertFileContent } from '../wailsjs/go/main/App.js';
...
function hlCode(code, lang) {
  ...
  // step 5 — function names (skip known keywords)
  var kwSet = {};
  if(kw) {
    kw.source.replace(/\b([a-zA-Z_]\w*)\b/g, function(_, w){ kwSet[w] = true; });
  }
  code = code.replace(/\b([a-zA-Z_]\w*)\s*(?=\()/g, function(m, name){
    if(kwSet[name]) return m;
...
async function importPDF() {
  try {
      // Open file dialog for PDF only
      var path = await OpenImportFile();
      if (!path) return;

      // Check extension
      var name = path.split(/[\\/]/).pop();
      if (!name.toLowerCase().endsWith('.pdf')) {
          toast('Only PDF files are supported for now');
          return;
      }
      ...
      var content = await ReadFileBytes(path);

      // Call Go converter
      var result = await ConvertFileContent(name, content);

      if (result.success) {
          // Open as new file in editor
          var fileName = name.replace(/\.[^.]+$/, '') + '.md';
          files.push({
              name: fileName,
              content: result.markdown,
              dirty: false,
              path: null  // converted content, no disk path yet
          });
          setActive(files.length - 1);
          renderSidebar();
          toast('Imported ' + name);
...
document.addEventListener('drop', async function(e){
  ...
  document.getElementById('btn-import').addEventListener('click', importPDF);
